...
export function BrokerPerformanceGrid({ sales, leads, properties, selectedMonth, selectedYear, brokers }: BrokerPerformanceGridProps) {
  ...
  const stats = useMemo(() => {
    ...
    return brokers.map(brokerName => {
      ...
      const brokerDealsAll = sales.filter(s => isMatch(s.vendedor));
      const brokerSalesAll = brokerDealsAll.filter(s => !normalize(s.tipo || '').includes('loca') && !normalize(s.tipo || '').includes('aluguel'));
      const brokerRentalsAll = brokerDealsAll.filter(s => normalize(s.tipo || '').includes('loca') || normalize(s.tipo || '').includes('aluguel'));
            ...
            <TableHeader>
              <TableRow className="bg-muted/5">
                ...
                <TableHead className="text-center border-r text-xs uppercase relative">
                   ...
                   Leads / {performanceView === 'venda' ? 'Venda' : 'Loc.'}
                </TableHead>
                <TableHead className="text-center border-r text-xs uppercase relative">
                   <span className="absolute top-1 left-1 text-[8px] opacity-40">%</span>
